...
++ explain that we need to deal with missing values that were introduced because of preprocessing (lags, etc). We need to impute or to drop the missing values. We'll drop them because the amount of NA's is relatively small
