...
# Constructing a target matrix for multiple horizons

To make multi-step predictions, we require separate target series for each forecast horizon. This setup ensures that the model can output forecasts for three consecutive months. Since we’re making forecasts for multiple time steps into the future, the target variable needs to include values for each future point within the forecast interval. We achieve this by creating a matrix with separate columns for each horizon (three periods in this case). Each column in the target matrix represents one step forward in time, while rows without sufficient future values are dropped. This process may cause misalignment between the features and targets, which is resolved by selecting only rows that are common to both.
...
# Summary 
This tutorial demonstrates a structured approach to multi-step forecasting using a pipeline in Python. By leveraging feature engineering, linear regression, and the multi-output capability of scikit-learn, we can predict multiple periods ahead with relative ease. The RMSE scores indicate the model's performance across the forecast horizons, and this direct approach can serve as a baseline or be further enhanced with more complex models for real-world applications requiring multi-period forecasting.